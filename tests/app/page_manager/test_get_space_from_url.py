...
def test_good_input(page_manager_object):
    ...
    assert page_manager_object.get_space_from_url(
        'http://buic-confluence.conti.de:8090/display/~uidj5418/Page+1') == '~uidj5418'
    assert page_manager_object.get_space_from_url(
        'http://buic-confluence.conti.de:8090/display/SWPT/Gerrit+Sandbox') == 'SWPT'
    assert page_manager_object.get_space_from_url(
        'http://buic-confluence:8090/display/AHU/FordAHU_1.00.05') == 'AHU'
    assert page_manager_object.get_space_from_url(
        'http://buic-confluence.conti.de:8090/display/TS/Automated+Release') == 'TS'
    assert page_manager_object.get_space_from_url(
        'http://wiki-id.conti.de/display/CIPS/CIPS+V02.01.00') == 'CIPS'
    assert page_manager_object.get_space_from_url(
        'http://buic-confluence.conti.de:8090/display/ATP/Automation') == 'ATP'
    assert page_manager_object.get_space_from_url(
        'http://buic-confluence.conti.de:8090/display/DFSSN/DFSS+Network') == 'DFSSN'
    assert page_manager_object.get_space_from_url(
        'http://buic-confluence.conti.de:8090/display/GO/SW+Sync') == 'GO'
    assert page_manager_object.get_space_from_url(
        'http://buic-confluence.conti.de:8090/display/IIC/I+IC') == 'IIC'
...
def test_bad_input(page_manager_object):
    ...
    with pytest.raises(Exception):
        page_manager_object.get_space_from_url(
